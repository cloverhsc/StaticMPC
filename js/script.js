...
var App = (function () {
            ...
            watch: {
                ...
                'setting.keyboard': function (val, oldVal) {
                    if (!_this.vueChangeByMe) {
						...
						// 把網頁上面的Layout options 選到 language[0]的選項
                ...
                setting: {
					...
					deep: true			// 深度觀察 vuePageData.setting 的所有屬性變化。
                ...
                data: {
					...
					deep: true			// 深度觀察 vuePageData.data 的所有屬性變化。
                }
            },
            methods: {
    ...
    App.prototype.keyboardInit = function () {
        ...
        $('#Vortexgear').on('click', '.tabBoxMacro .physicalKeyboard .key', function (e) {
			var sourceKey = $(e.target).data('key');	// get DOM attribute data-key value
            if (sourceKey == '0') {
                return;
            }
            var idx = findIdxFromObjArrayByKey(_this.vuePageData.data.macro, {
                profileIndex: _this.vuePageData.setting.profile,		// 對應到網頁上面的 Layer 選項
                sourceKey: sourceKey,									// data-key 值
                sourceLayer: _this.vuePageData.setting.layer			// 對應到網頁上面的 Func.key 選項
            });
    ...
    App.prototype.checkConflict = function (sourceKey) {
		console.log(this.vuePageData.data.macro);
		...
		console.log('---------------------------');
		console.log(this.vuePageData.data.keyChange);
		console.log(findIdxFromObjArrayByKey(this.vuePageData.data.keyChange, {
			profileIndex: this.vuePageData.setting.profile,
			sourceKey: sourceKey,
			sourceLayer: this.vuePageData.setting.layer
		}));
	...
	*/
	...
	/**
	 * 點擊 tab 為 macro時的鍵盤按鍵處理。
	 */
    App.prototype.macro = function (idx, sourceKey) {
		console.log(`App.prototype.macro idx: ${idx}`);
		console.log(`App.prototype.macro sourceKey: ${sourceKey}`);
        ...
        if (idx >= 0) {
            ...
            else if (this.checkConflict(sourceKey) != '') {		// 檢查按鍵是否已經被使用
    ...
    App.prototype.macroComplete = function () {
		var downKey = [];
		/* 
			先檢查 row event === '1' 代表按下，然後把按鍵名稱(row.key)丟到downKey array內
			相同的按鍵(row.key)，然後遇到相同按鍵但是 event === '0' 代表放開這個按鍵的行為
			再去把他從 downkey array中移除。這樣代表這個按鍵有被release掉。

			剩下在downKey 內的代表沒被release的按鍵。
		*/
    ...
    App.prototype.keyChangeComplete = function () {
		...
		this.clearActive();
		console.log(this.vuePageData.active);
    };
	...
	/**
	 * 在Func. key 分頁內點擊 Fn or Fn1 or Pn 檢查
	 * 按鍵如果已經有設定就
	 */
    ...
    App.prototype.convertToBytes = function () {
        var itemSize = this.vuePageData.data.macro.length * 2 + this.vuePageData.data.keyChange.length + this.vuePageData.data.functionSet.length;
        var cysHeader = {
        ...
        var _loop_1 = function (filterIndex) {
            //function set
            var filterFunctionSet = this_1.vuePageData.data.functionSet.filter(function (row) {
        ...
        var this_1 = this;
        for (var filterIndex = 1; filterIndex <= 5; filterIndex++) {
            _loop_1(filterIndex);
        }
        var bytes = [];
...
/** 
 * 檢查 array 裡面有沒有跟 filter相同的物件屬性
 * 不過他的寫法似乎array 只會有一個元素。
 * e.g.
 * array = [{
 * 	macroRepeat: "2",
 * 	macroType: "1",
